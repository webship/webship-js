...
module.exports.assertion = function (element, negativeCase, elementCss) {
  ...
  browser.element('css selector', element, function (result) {
    if (result.status === 0) {
      if (negativeCase) {
        return browser.assert.not.cssProperty(element, cssProperty, propertyVal);
      }
      return browser.assert.cssProperty(element, cssProperty, propertyVal);
    } else {
      browser.element('css selector', '#' + element, function (result) {
        if (result.status === 0) {
          if (negativeCase) {
                      ...
                      browser.elementIdText(elementsObj.ELEMENT, function (result) {
                        if (result.value == element) {
                          browser.getAttribute(elementsObj, "for", function (forVal) {
                            ...
                            if (negativeCase) {
                              return browser.assert.not.cssProperty('#' + forVal.value, cssProperty, propertyVal);
                            }
                            return browser.assert.cssProperty('#' + forVal.value, cssProperty, propertyVal);
                          });
                        }
                      })
                    });
